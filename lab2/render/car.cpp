...
void Car::setAnimation(const glm::vec3& start, const glm::vec3& end, float duration) {
    startPosition = start;
    endPosition = end;
    animationDuration = duration;
    elapsedTime = 0.0f; // Reset animation
}
...
void Car::update(double deltaTime) {
    if (animationDuration > 0.0f) {
        ...
        setPosition(glm::mix(startPosition, endPosition, t));

        setRotationTowards(endPosition);
